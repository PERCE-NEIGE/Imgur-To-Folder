...
class Imgur_Downloader(Imgur):
        ...
        if any(re.search(item, url) for item in imgur_base_extensions['album']):
            ...
            for item in imgur_base_extensions['subreddit']:
                ...
                if id is None and subreddit is not None:
                    self.download_subreddit(
                        subreddit, sort=sort, window=window, page=page, max_items=max_items)
    ...
    def download_tag(self, id, page=0, max_items=30):
        self._log.debug('Getting tag details')
        ...
        for item in items:
            ...
            if 'images' in item:
                ...
                for position, sub_image in enumerate(item['images'], start=1):
                    ...
                    self._log.info('Downloading tag: %s' % title)
    ...
    def download_gallery(self, id):
        ...
        self._log.debug('Getting Gallery details')
        ...
        if 'images' in album:
            self._log.info('Downloading gallery %s' % album['id'])
            for position, image in enumerate(album['images'], start=1):
                image_link, filetype = self.get_image_link(image)
            ...
            self._log.info('Downloading gallery image: %s' % filename)
            self.download(filename, image_link, path)

    def download_subreddit(self, subreddit, sort='time', window='day', page=0, max_items=30):
        self._log.debug("Getting subreddit details")
        subreddit_data = []
        response = self.get_subreddit_gallery(
            subreddit, sort=sort, window=window, page=page)['data']
        ...
        self._log.debug("Sending subreddit items to parse_id")
    ...
    def download_subreddit_gallery(self, subreddit, id):
        ...
        self._log.debug('Getting subreddit gallery details')
        ...
        self._log.info('Downloading subreddit gallery image: %s' % title)
